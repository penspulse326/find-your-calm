...
<template>
  <div
    class="flex-1 flex flex-col items-center justify-center p-8 text-center bg-linear-to-b from-gray-900 to-black text-white h-full relative"
  >
    <div class="space-y-6 z-10 w-full relative">
      ...
      <div
        class="text-white/70 leading-relaxed text-sm text-left bg-white/5 p-6 rounded-xl border border-white/10 shadow-lg min-h-[120px]"
      >
        <p v-if="score <= 4">
          你的內心是一片平靜的湖泊。繼續保持目前的步調，每天 5 分鐘正念。
        </p>
        <p v-else-if="score <= 9">
          有些波浪，但你還能掌舵。建議減少咖啡因，並嘗試
          <strong class="text-white/90">4-7-8 呼吸法</strong>。
        </p>
        <p v-else-if="score <= 14">
          你似乎迷失了方向。請實行數位排毒，嘗試
          <strong class="text-white/90">箱式呼吸</strong> 來找回主控權。
        </p>
        <p v-else>
